...
export default function ParentRegister() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { firstName, lastName, email, country, childName, heard, password } = form;
    if (!firstName || !lastName || !email || !country || !childName || !heard || !password) {
    ...
    <div className="relative min-h-screen w-full overflow-x-hidden text-white" style={{ backgroundImage: unifiedBackground, backgroundColor: '#034242' }}>
      ...
      <div className="pt-32 pb-16 px-6 flex items-start justify-center">
        <div className="w-full max-w-3xl bg-white text-[#0f5a56] rounded-3xl shadow-xl p-6 md:p-8 border border-white/20">
          <h1 className="text-2xl font-bold mb-2">Sign up as Parent</h1>
          <p className="text-sm text-[#0f5a56]/80 mb-6">Create a parent account to manage and oversee your child's learning. You'll find info about the proficiency exam in the dashboard after sign-up.</p>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <input required value={form.childName} onChange={(e)=>onChange('childName', e.target.value)} className="w-full bg-transparent border-b border-gray-300 focus:border-[#0f5a56] outline-none py-2" />
            </div>
            <div>
            ...
            <div className="md:col-span-2 text-center text-xs text-[#0f5a56]/70">
              ...
              <button type="button" onClick={()=> (window.location.hash = '#/login?role=parent')} className="ml-2 underline hover:no-underline text-[#0f5a56]">Log in</button>
            </div>
          </form>
        </div>
      </div>
